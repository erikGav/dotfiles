...
function print_block {
    INDEX=$(pactl list sinks | grep -E '^\s+\*|index:' | grep -B1 'State: RUNNING' | grep -o '[0-9]\+' | head -1)
    [[ -z "$INDEX" ]] && INDEX=$(pactl list sinks short | awk '{print $1}' | head -1)
    VOL=$(pactl get-sink-volume @DEFAULT_SINK@ | grep -oP '\d+%' | head -1)
    VOL="${VOL%?}"
    MUTED_RAW=$(pactl get-sink-mute @DEFAULT_SINK@ | grep -oP '(yes|no)')
    MUTED="muted: ${MUTED_RAW}"
    DEFAULT_SINK_NAME=$(pactl get-default-sink)
    NAME=$(pactl list sinks | awk -v sink="$DEFAULT_SINK_NAME" '
        /^Sink #/{found=0} /Name: / && $0 ~ sink {found=1}
        found && /device\.description/{gsub(/.*= "|"/,""); print; exit}')
    [[ -z "$NAME" ]] && NAME=$DEFAULT_SINK_NAME
